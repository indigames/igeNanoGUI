...
static void glnvg__renderFlush(void* uptr)
{
	...
	if (gl->ncalls > 0) {
	// [IGE]: Backup OpenGL values
		int viewport_backup[4], scissor_backup[4];
		...
		glGetIntegerv(GL_SCISSOR_BOX, scissor_backup);

		GLboolean depth_write;
		glGetBooleanv(GL_DEPTH_WRITEMASK, &depth_write);
		depth_write_backup = depth_write;

		depth_test_backup = glIsEnabled(GL_DEPTH_TEST);
		scissor_test_backup = glIsEnabled(GL_SCISSOR_TEST);
		cull_face_backup = glIsEnabled(GL_CULL_FACE);
		blend_backup = glIsEnabled(GL_BLEND);
		GLint blend_src_backup;
		GLint blend_dst_backup;
		glGetIntegerv(GL_BLEND_SRC_ALPHA, &blend_src_backup);
		glGetIntegerv(GL_BLEND_DST_ALPHA, &blend_dst_backup);
	// [/IGE]
	...
	// [IGE]: Restore
		glViewport(viewport_backup[0], viewport_backup[1],
					viewport_backup[2], viewport_backup[3]);
		glScissor(scissor_backup[0], scissor_backup[1],
					scissor_backup[2], scissor_backup[3]);

		if (depth_test_backup)
			glEnable(GL_DEPTH_TEST);
		else
			glDisable(GL_DEPTH_TEST);

		glDepthMask(depth_write_backup);

		if (scissor_test_backup)
		...
		glBlendFunc(blend_src_backup, blend_dst_backup);
	// [/IGE]
	}
